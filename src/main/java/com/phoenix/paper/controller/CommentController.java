...
public class CommentController {
    ...
    @ApiImplicitParams({
            ...
            @ApiImplicitParam(name = "type",value = "对象类型(0:笔记 1:评论)",required = true,paramType = "query",dataType = "Integer"),
            @ApiImplicitParam(name = "content",value = "内容",required = true,paramType = "query",dataType = "String"),})
    public Result addComment(@NotNull @RequestParam("id") Long objectId, @NotNull @Min(value=0,message="无效类型") @Max(value = 1,message ="无效类型" ) @RequestParam("type")Integer objectType, @RequestParam("content")String content) {
        try{
    ...
    public Result getCommentList(@NotNull @RequestParam("id") Long objectId,
                                 @Min(value = 0,message = "无效类型") @Max(value = 1,message = "无效类型") @NotNull @RequestParam("type")Integer objectType,
                                 @NotNull @Min(value = 0) @RequestParam("pageSize") Integer pageSize,
                                 @NotNull @Min(value = 1) @RequestParam("pageNum")Integer pageNum) {
