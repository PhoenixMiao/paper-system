...
public interface NoteMapper extends BaseMapper<Note> {
    ...
    @Select("SELECT id,author_id,title,cover,create_time,like_number,collect_number FROM note LEFT JOIN user ON note.author_id = user.id WHERE user.nickname LIKE %#{author}%;")
    List<BriefNote> getBriefNoteListByAuthor(@Param("author") String author);

    @Select("SELECT id,author_id,title,cover,create_time,like_number,collect_number FROM note WHERE title LIKE #{title}%;")
    List<BriefNote> getBriefNoteListByTitle(@Param("author") String title);

    @Select("SELECT id,author_id,title,cover,create_time,like_number,collect_number FROM note;")
