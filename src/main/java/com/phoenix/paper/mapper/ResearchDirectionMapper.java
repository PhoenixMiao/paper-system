...
public interface ResearchDirectionMapper extends MyMapper<ResearchDirection> {
    @Select("SELECT id,name,is_leaf FROM research_direction WHERE father_id = #{father_id} AND LEN(delete_time) = 0;")
    List<BriefNode> getSons(@Param("father_id")Long father_id);

    @Select("SELECT id FROM research_direction WHERE root_id = #{root_id} AND path LIKE #{path} AND LEN(delete_time) = 0;")
