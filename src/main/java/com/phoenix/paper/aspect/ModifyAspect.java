...
public class ModifyAspect {
    ...
    @Around("@annotation(com.phoenix.paper.annotation.Auth)")
    public Object doAroundModify(ProceedingJoinPoint joinPoint) throws Throwable {

        SessionData sessionData = sessionUtil.getSessionData();

        AssertUtil.notNull(sessionData, CommonErrorCode.INVALID_SESSION);

        AssertUtil.isTrue(sessionData.getCanModify() == 1 || sessionData.getType() == 1, CommonErrorCode.CAN_NOT_COMMENT);
